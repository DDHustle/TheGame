...
namespace TheGame {
	class Polymesh {
		...
		public Polymesh Interconnect(Vector2f[] S2) {
			...
			return new Polymesh(Pts);
		}

		public ConvexShape ToShape() {
			ConvexShape S = new ConvexShape((uint)Points.Length);
			for (int i = 0; i < Points.Length; i++)
				S.SetPoint((uint)i, Points[i]);
			return S;
		}
	}
}